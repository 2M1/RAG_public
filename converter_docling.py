        ...
        files = list(
            map(
                ...
                filter(
                    ...
                    lambda entry: entry.is_file() and os.path.splitext(entry)[-1] == ".pdf",
...
def convert_dir_files(source_directory: Path, output_directory: Path) -> None:
    ...
    input_files: dict[str: list[str]] = _parse_collection_file_struct_from_dir(source_directory)
    ...
    for dir_name, file_names in input_files.items():
        for file in file_names:
            if os.path.splitext(file)[1] == ".pdf":  # Process only PDF files
                ...
                output_file_path = os.path.join(output_directory / dir_name, output_file_name)
